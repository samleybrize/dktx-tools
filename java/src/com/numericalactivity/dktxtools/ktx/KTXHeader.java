...
public abstract class KTXHeader {
    ...
    public String toString() {
        return String.format(
            ...
            getClass().getCanonicalName(), _glType, _glTypeSize, _glFormat, _glInternalFormat, _glBaseInternalFormat, _pixelWidth, _pixelHeight, _pixelDepth,
            _numberOfArrayElements, _numberOfFaces, _numberOfMipmapLevels, _bytesOfKeyValueData
        );
    }
    ...
    public static class Writer extends KTXHeader {
        ...
        protected void check() throws KTXFormatException {
            ...
            if (_numberOfMipmapLevels > 1 && numberOfMipmaps != _numberOfMipmapLevels) {
                throw new KTXFormatException(String.valueOf(numberOfMipmaps) + " mipmap levels must be set, but " + String.valueOf(_numberOfMipmapLevels) + " are set");
            }
            ...
            if (_glTypeSize > 0 && (1 != _glTypeSize && 2 != _glTypeSize && 4 != _glTypeSize)) {
                throw new KTXFormatException("glTypeSize must be either 1, 2 or 4 for non compressed textures. " + String.valueOf(_glTypeSize) + " given");
            }

            // on contrôle la valeur de glType
            if (_glType > 0 && TextureFormat.GL_UNSIGNED_BYTE != _glType) {
                throw new KTXFormatException("glType must be 0x" + Integer.toHexString(TextureFormat.GL_UNSIGNED_BYTE) + " (TextureFormat.GL_UNSIGNED_BYTE) for nno compressed textures");
            }

            // on contrôle la valeur de format
            if (_glType > 0 && (
                    TextureFormat.GL_ALPHA != _glFormat
                    && TextureFormat.GL_LUMINANCE != _glFormat
                    && TextureFormat.GL_LUMINANCE_ALPHA != _glFormat
                    && TextureFormat.GL_RGB != _glFormat
            ...
            if ((0 == _glType && _glFormat > 0) || (_glType > 0 && 0 == _glFormat)) {
                throw new KTXFormatException("Both glType and glFormat must equal 0 for non compressed textures, and both must not equal 0 for compressed textures");
            }

            // on contrôle la valeur de internalFormat
            if (0 == _glType && (
                    TextureFormat.GL_ATC_RGB_AMD != _glInternalFormat
                    ...
                    && TextureFormat.GL_ETC1_RGB8 != _glInternalFormat
                    && TextureFormat.GL_COMPRESSED_RGB_S3TC_DXT1_EXT != _glInternalFormat
            ...
            if (TextureFormat.GL_ALPHA != _glBaseInternalFormat
                    && TextureFormat.GL_LUMINANCE != _glBaseInternalFormat
                    && TextureFormat.GL_LUMINANCE_ALPHA != _glBaseInternalFormat
                    && TextureFormat.GL_RGB != _glBaseInternalFormat
