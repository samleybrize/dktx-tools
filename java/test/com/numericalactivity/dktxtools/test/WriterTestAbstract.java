...
abstract public class WriterTestAbstract {
    ...
    protected static ByteBuffer[] _compressedPvrtcTextureBuffer;
    ...
    public WriterTestAbstract() {
        ...
        try {
            ...
            _compressedPvrtcTextureBuffer   = new ByteBuffer[FILE_NUMBER_OF_MIPMAPS];
            BufferedImage img;
            File file;
            FileInputStream in;
            short width;
            ...
            for (byte i = 0; i < FILE_NUMBER_OF_MIPMAPS; i++) {
                ...
                // texture compressée en PVRTC
                file                                = new File(FILE_COMPRESSED_TEXTURE_BASE + String.valueOf(i) + ".pvrtc");
                in                                  = new FileInputStream(file);
                compressedData                      = new byte[(int) file.length()];
                in.read(compressedData);
                _compressedPvrtcTextureBuffer[i]    = BufferUtils.getByteBuffer(compressedData);
